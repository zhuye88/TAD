...
class LSHiForest(LSHForest):
    ...
    def __init__(self, lsh_family="L2SH", num_trees=100, granularity=1):
        if lsh_family == "ALSH":
            LSHForest.__init__(
                self, num_trees, VSSampling(num_trees), AngleLSH(), granularity=1
            )
        elif lsh_family == "L1SH":
            LSHForest.__init__(
                self, num_trees, VSSampling(num_trees), E2LSH(norm=1), granularity=1
            )
        elif lsh_family == "KLSH":
            LSHForest.__init__(
                self, num_trees, VSSampling(num_trees), KernelLSH(), granularity=1
            )
        else:
            LSHForest.__init__(
                self, num_trees, VSSampling(num_trees), E2LSH(norm=2), granularity=1
            )
