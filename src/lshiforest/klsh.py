...
class KernelLSH(LSH):
    ...
    def fit(self, data):
        ...
        train_data_size = min(
            int(np.ceil(pow(data_size, self._para_p_exp))), self._para_p
        )
        ...
        if self._kernel_func == "rbf":
            ...
            # 			elif type(self._kernel_kwds) == tp.StringType and self._kernel_kwds == 'range':
                ...
                self._kernel_kwds = pow(
                    10, np.random.uniform(np.log10(0.1 / d), np.log10(1.0 / d))
                )
    ...
    def _compute_kernel(self, X, Y):
        ...
        if self._kernel_func == "puk":
            ...
            return np.power(
                (1.0 + 8 * gamma * (np.power(2.0, 1.0 / omega) - 1.0) * squared_diff),
                -omega,
            )
    ...
    def _extend_weight_pool(self, extended_size):
        ...
        size = min(
            extended_size,
            int(comb(self._para_p, self._para_t)) ** self._nbits - len(self._e_s_pool),
        )

        if size > 0 and len(self._e_s_pool) == 0:
            new_e_s = self._generate_e_s()
            self._e_s_pool.append(new_e_s)
            self._weight_pool.append(
                np.sqrt((self._para_p - 1.0) / self._para_t) * self._K_half * new_e_s
            )
            size -= 1
        ...
        for i in range(size):
            ...
            while repeated == True:
                ...
                if repeated == False:
                    self._e_s_pool.append(new_e_s)
                    self._weight_pool.append(
                        np.sqrt((self._para_p - 1.0) / self._para_t)
                        * self._K_half
                        * new_e_s
                    )
    ...
    def _generate_e_s(self):
        ...
        i = np.array(
            [
                np.random.choice(self._para_p, self._para_t, False)
