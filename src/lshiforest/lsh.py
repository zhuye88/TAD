...
class E2LSH(LSH):
    ...
    def get_hash_value(self, x, hash_index):
        ...
        return int(
            np.floor(
                (np.dot(x, self.A_array[hash_index]) + self.B_array[hash_index])
                / self._bin_width
            )
        )
...
class MinLSH(LSH):
    ...
    def __init__(self, n_dimensions, default_pool_size=50):
        ...
        self._hash_parameters_pool = np.c_[
            np.random.randint(0, np.iinfo("i").max, default_pool_size),
            np.random.randint(0, np.iinfo("i").max, default_pool_size),
            np.random.randint(0, np.iinfo("i").max, default_pool_size),
        ]
            ...
            while repeated == True:
                ...
                for j in range(0, cur_len):
                    if np.array_equal(new_para, self._hash_parameters_pool[j]):
                        ...
                        break
                if repeated == False:
                    self._hash_parameters_pool = np.append(
                        self._hash_parameters_pool, [new_para], axis=0
                    )
                    cur_len += 1

        result = np.iinfo("i").max
        for i in x:
            hash_value = self._permutationHash(
                i,
                self._hash_parameters_pool[hash_index][0],
                self._hash_parameters_pool[hash_index][1],
                self._hash_parameters_pool[hash_index][2],
            )
