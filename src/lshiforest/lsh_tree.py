...
class LSHTree:
    ...
    def _recursive_build(self, data, depth_limit, lof=0, hash_func_index=0):
        ...
        if n_samples == 1 or hash_func_index > depth_limit:
            ...
            for key in partition.keys():
                ...
                children[key] = self._recursive_build(
                    child_data,
                    depth_limit,
                    min(0.0, (children_count[key] - mean) / std),
                    cur_index + 1,
                )
    ...
    def _recursive_display(self, lsh_node, leftStr=""):
        ...
        print(
            ...
            + str(len(leftStr))
            + ","
            + str(lsh_node._hash_func_index)
            + "):"
            + str(lsh_node._data_size)
            + ":"
            + str(lsh_node._children_count)
            + ","
            + str(lsh_node._lof)
        )
    ...
    def _recursive_get_search_depth(self, lsh_node, cur_depth, granularity, point):
        ...
        if not children:
            ...
            adjust_factor = self.get_random_path_length_symmetric(
                lsh_node.get_data_size()
            )
            return (
                cur_depth
                * np.power(1.0 * real_depth / max(cur_depth, 1.0), granularity)
                + adjust_factor
            )
        else:
            ...
            if key in children.keys():
                return self._recursive_get_search_depth(
                    children[key], cur_depth + 1, granularity, point
                )
            else:
                cur_depth = cur_depth + 1
                real_depth = lsh_node._hash_func_index + 1
                return cur_depth * np.power(
                    1.0 * real_depth / max(cur_depth, 1), granularity
                )
    ...
    def get_random_path_length_symmetric(self, num_samples):
        if num_samples <= 1:
            ...
            return (
                np.log(num_samples) + np.log(self._branch_factor - 1.0) + 0.5772
            ) / np.log(self._branch_factor) - 0.5
