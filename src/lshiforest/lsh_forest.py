    ...
    def build(self, data):
        ...
        for i in range(self._num_trees):
            ...
            if self._sampler._bagging != None:
                transformed_data = self._sampler._bagging_instances[
                    i
    ...
    def decision_function(self, data):
        ...
        for i in range(data_size):
            ...
            for j in range(self._num_trees):
                ...
                if self._sampler._bagging != None:
                    transformed_data = (
                        self._sampler._bagging_instances[j]
                        .get_transformed_data(np.mat(data[i]))
                        .A1
                    )
                d_depths.append(
                    self._trees[j].predict(self._granularity, transformed_data)
                )
