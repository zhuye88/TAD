...
def ProduceMultiTS(str):
    ts_names = str.split("+")
    df = np.array(
        pd.read_csv("RealDatasets/MBA_ECG_" + ts_names[0] + ".ts", header=None)
    )[: (int)(1e5)].reshape(-1)
    ...
    for i, ts_name in enumerate(ts_names):
        if i == 0:
            continue
        df = np.array(
            pd.read_csv("RealDatasets/MBA_ECG_" + ts_name + ".ts", header=None)
        )[: (int)(1e5)].reshape(-1)
        pos_list = getDiscordList(
            "RealDatasets/ANNOTATIONS/MBA_" + ts_name + "(1-100K).txt"
        ) + (int)(i * 1e5)
...
def get_scoreCycleList_from_sliding(cycle, win_size, labels, score_list, zone=None):
    ...
    for i in range(len(result)):
        ...
        if lo >= len(score_list):
            print("have 0")
            break
        hi = min(len(score_list), (i + 1) * cycle + zone - win_size + 1)
        assert lo < hi
        # if labels[i]==1:
...
def findthredsomeInSlidingWindowIDK(scorelist, redlist, width):
    ...
    for i in range(1, len(redlist)):
        if redlist[i][0] - redlist[i - 1][1] >= width:
            tem = scorelist[
                range(redlist[i - 1][1], min(redlist[i][0] - width + 1, len(scorelist)))
            ]
            # if max(tem)>maxi:
            #     maxi=max(tem)
            #     index=i
...
def step_subsequence(time_series, L, S=1):  # Window len = L, Stride len/stepsize = S
    ...
    return np.lib.stride_tricks.as_strided(
        time_series, shape=(nrows, L), strides=(S * n, n)
    )
